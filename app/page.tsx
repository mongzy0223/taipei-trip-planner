...
const supabase=createClient('https://lfslniiyagnyssxummml.supabase.co','sb_publishable_ltaNA7nnVozoSCOcZIjg');
...
const applySession=async(session:Session)=>{setUserEmail(session.user.email||'SIGNED IN');setSyncStatus('SENDING');const{data,error}=await supabase.from('trip_progress').select('checklist,packing').eq('user_id',session.user.id).maybeSingle();if(error){setSyncStatus('ERROR');return}if(data){setDone(data.checklist||{});setPackDone(data.packing||{});localStorage.setItem('taipei-checklist-v2',JSON.stringify(data.checklist||{}));localStorage.setItem('taipei-packing-v1',JSON.stringify(data.packing||{}));setSyncStatus('SYNCED')}else{const{error:saveError}=await supabase.from('trip_progress').upsert({user_id:session.user.id,checklist:done,packing:packDone,updated_at:new Date().toISOString()});setSyncStatus(saveError?'ERROR':'SYNCED')}};
...
const saveCloud=async(nextDone:Record<number,boolean>,nextPack:Record<number,boolean>)=>{const{data:{session}}=await supabase.auth.getSession();if(!session)return;setSyncStatus('SENDING');const{error}=await supabase.from('trip_progress').upsert({user_id:session.user.id,checklist:nextDone,packing:nextPack,updated_at:new Date().toISOString()});setSyncStatus(error?'ERROR':'SYNCED')};
...
const sendMagicLink=async()=>{if(!email.trim())return;setSyncStatus('SENDING');const{error}=await supabase.auth.signInWithOtp({email:email.trim(),options:{emailRedirectTo:'https://mongzy0223.github.io/taipei-trip-planner/'}});setSyncStatus(error?'ERROR':'SYNCED')};
