...
const supabase=createClient('https://lfslniiyagnyssxummml.supabase.co','eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxmc2xuaWl5YWdueXNzeHVtbW1sIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODgwMDg1MTksImV4cCI6MjEwMzU4NDUxOX0.YKsDMEyR7ngY_YosGbnmLLUgDg84PLU49Y4FZKR_qck');
...
const[active,setActive]=useState(0);const[view,setView]=useState<'plan'|'ready'|'flights'|'packing'|'more'>('plan');const[done,setDone]=useState<Record<number,boolean>>({});const[packDone,setPackDone]=useState<Record<number,boolean>>({});const[loaded,setLoaded]=useState(false);const[copied,setCopied]=useState(false);const[email,setEmail]=useState('');const[userEmail,setUserEmail]=useState('');const[syncStatus,setSyncStatus]=useState<'LOCAL'|'SENDING'|'SYNCED'|'ERROR'>('LOCAL');const[syncMessage,setSyncMessage]=useState('');const day=days[active];
...
const applySession=async(session:Session)=>{setUserEmail(session.user.email||'SIGNED IN');setSyncStatus('SENDING');setSyncMessage('Loading cloud progress…');const{data,error}=await supabase.from('trip_progress').select('checklist,packing').eq('user_id',session.user.id).maybeSingle();if(error){setSyncStatus('ERROR');setSyncMessage(error.message);return}if(data){setDone(data.checklist||{});setPackDone(data.packing||{});localStorage.setItem('taipei-checklist-v2',JSON.stringify(data.checklist||{}));localStorage.setItem('taipei-packing-v1',JSON.stringify(data.packing||{}));setSyncStatus('SYNCED');setSyncMessage('Cloud progress loaded.')}else{const{error:saveError}=await supabase.from('trip_progress').upsert({user_id:session.user.id,checklist:done,packing:packDone,updated_at:new Date().toISOString()});setSyncStatus(saveError?'ERROR':'SYNCED');setSyncMessage(saveError?.message||'Local progress uploaded.')}};
useEffect(()=>{if(!loaded)return;supabase.auth.getSession().then(({data})=>{if(data.session)applySession(data.session)});const{data:{subscription}}=supabase.auth.onAuthStateChange((_event,session)=>{if(session)applySession(session);else{setUserEmail('');setSyncStatus('LOCAL')}});return()=>subscription.unsubscribe()},[loaded]);
const saveCloud=async(nextDone:Record<number,boolean>,nextPack:Record<number,boolean>)=>{const{data:{session}}=await supabase.auth.getSession();if(!session)return;setSyncStatus('SENDING');setSyncMessage('Saving…');const{error}=await supabase.from('trip_progress').upsert({user_id:session.user.id,checklist:nextDone,packing:nextPack,updated_at:new Date().toISOString()});setSyncStatus(error?'ERROR':'SYNCED');setSyncMessage(error?.message||'Saved across devices.')};
const toggle=(i:number)=>{const next={...done,[i]:!done[i]};setDone(next);localStorage.setItem('taipei-checklist-v2',JSON.stringify(next));saveCloud(next,packDone)};
const togglePack=(i:number)=>{const next={...packDone,[i]:!packDone[i]};setPackDone(next);localStorage.setItem('taipei-packing-v1',JSON.stringify(next));saveCloud(done,next)};
const sendMagicLink=async()=>{if(!email.trim())return;setSyncStatus('SENDING');setSyncMessage('Sending login email…');const{error}=await supabase.auth.signInWithOtp({email:email.trim(),options:{emailRedirectTo:'https://mongzy0223.github.io/taipei-trip-planner/'}});setSyncStatus(error?'ERROR':'SYNCED');setSyncMessage(error?.message||'Login link sent — check your email.')};
...
<nav className="site-tabs" aria-label="Trip sections">{([['plan','PLAN','行程'],['ready','READY','準備'],['flights','FLIGHTS','航班'],['packing','PACK','行李'],['more','MORE','備用']] as const).map(([key,en,zh])=><button key={key} className={view===key?'active':''} onClick={()=>{setView(key);window.scrollTo({top:350,behavior:'smooth'})}} aria-current={view===key?'page':undefined}><b>{en}</b><span>{zh}</span></button>)}</nav>
<section className="today-bar"><div><p className="eyebrow">TODAY MODE</p><b>{daysAway>0?`${daysAway} DAYS TO GO`:`DAY ${todayIndex+1} · ${days[todayIndex].title}`}</b><span>{daysAway>0?'行程開始前可先預覽 Day 1':'下一站資料已準備好'}</span></div><button onClick={()=>{setView('plan');setActive(todayIndex);document.querySelector('.planner')?.scrollIntoView({behavior:'smooth'})}}>OPEN TODAY <span>→</span></button></section>
<section className="utility prep-tools"><header className="section-head"><div><p className="eyebrow">01 / PRE-TRIP</p><h2>GET READY</h2><small className="zh-subtitle">出發前 Checklist 及預約</small></div><p>先完成兩程 Check-in 同必要準備；酒店已標示為完成預訂。</p></header><aside className="cloud-sync"><div><span>CLOUD SYNC · {syncStatus}</span><b>{userEmail||'跨裝置同步勾選進度'}</b><p>{syncMessage||(userEmail?'已登入；Checklist 與 Packing 會自動儲存。':'用同一個 email 登入手機及電腦。')}</p></div>{userEmail?<button onClick={()=>supabase.auth.signOut()}>SIGN OUT</button>:<div className="sync-form"><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="your@email.com" aria-label="Email for cloud sync"/><button onClick={sendMagicLink}>SEND LOGIN LINK</button></div>}</aside><div className="tools-grid"><article className="checklist"><div className="tool-title"><span>CHECKLIST</span><b>{Object.values(done).filter(Boolean).length} / {checklist.length}</b></div>{loaded&&checklist.map((item,i)=><button key={item} className={done[i]?'checked':''} onClick={()=>toggle(i)}><i>{done[i]?'✓':''}</i><span>{item}</span></button>)}</article><article className="booking-list"><div className="tool-title"><span>RESERVATIONS</span><b>{String(reservations.length).padStart(2,'0')}</b></div>{reservations.map(r=><a key={r.title} href={r.href} target="_blank" rel="noreferrer"><small>{r.type}</small><div><b>{r.title}</b><span>{r.detail} · {r.meta}</span></div><i>↗</i></a>)}</article></div></section>
